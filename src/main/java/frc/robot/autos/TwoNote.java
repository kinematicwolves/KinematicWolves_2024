...
public class TwoNote extends SequentialCommandGroup {
    public TwoNote(Swerve s_Swerve, Arm s_Arm, Intake s_Intake, Lighting s_Lighting){
        ...
        Trajectory middleNote =
            TrajectoryGenerator.generateTrajectory(
              ...
              new Pose2d(1.6, 0, new Rotation2d(-0.05)),  
            
                config);

        Trajectory middleNoteS =
            TrajectoryGenerator.generateTrajectory(
                // Start at the origin facing the +X direction
                new Pose2d(0, 0, new Rotation2d(0)),
              List.of(new Translation2d(-.5, 0)),
              new Pose2d(-1.65, 0, new Rotation2d(-0.05)),
                config);
        ...
        addCommands(
            ...
            // new AutoStowNote(s_Intake, s_Arm),
            // new TimedShootNote(s_Intake, s_Arm, s_Lighting, 20900, 30, 15, IntakeProfile.kWristDefaultOutput, 1.9),
            new InstantCommand(() -> s_Swerve.setPose(middleNoteS.getInitialPose())),
            swerveControllerCommand2,
            new InstantCommand(() -> s_Swerve.drive(new Translation2d(0,0), 0, false, true)),
            // new AutoIntakePlus(s_Intake, s_Arm, s_Lighting),
            // new AutoStowNote(s_Intake, s_Arm),
            new TimedShootNote(s_Intake, s_Arm, s_Lighting, ArmProfile.kpivotSpeakerPos, 30, 15, IntakeProfile.kWristDefaultOutput, 1.9),
